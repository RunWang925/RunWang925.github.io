...
## 🤔待解决问题
1. 图床部署后，经 CDN 加速域名上传图片时存在异常：上传请求触发后页面持续卡顿，无法正常返回图片访问链接，但实际文件已成功上传至存储服务，后台可正常查询。

2. 当前临时适配方案：采用源站未加速域名执行图片上传操作，然后设置中配置自定义链接为 CDN 加速地址（具体配置见下图）。
![image.png](https://cdnimg-doge.814925.xyz/file/waline/1764472406380_image.png)
